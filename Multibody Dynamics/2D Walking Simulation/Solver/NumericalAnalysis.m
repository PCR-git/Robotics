...
function [T,Y,Te,Ye,PHI_T,qpp] = NumericalAnalysis(f_DAE,f_Evalconstraints,Yn,t_final,n,m,rwi,rwj,WPtol,Go,CatchDistance)
...
% Reltol and Abstol give reasonable accuracy for values of 1e-5 or lower
...
function [switchWP,isterminal,direction] = WayPtContact(~,Y)
...
%      A = [q(1)+v(1);q(2)+v(2)];
     ...
     if Go == 1
        % switchWP = 0 when point A reaches desired position
        % f_SwitchWP determines whether the robot should switch to the next way point
        switchWP = f_SwitchWP(Y,n,m,rwi,rwj,WPtol,CatchDistance);
     elseif Go == 0
        switchWP = 1;  % Prevents event from being triggered
     end
        isterminal = 1;  % Terminate integration at event? (1=y,0=n)
